class Schedule < ActiveRecord::Base
  ...
  def should_send_notification?
    ...
    # Check if the schedule is approximately 30 minutes away (with 1.5 minute tolerance)
    time_until_schedule = (schedule_at - Time.current) / 1.minute
    time_until_schedule >= 28.5 && time_until_schedule <= 31.5
  end
  ...
  def self.needing_notification
    where(notification_sent: false)
      .where('schedule_at > ? AND schedule_at <= ?', 28.5.minutes.from_now, 31.5.minutes.from_now)
  end
